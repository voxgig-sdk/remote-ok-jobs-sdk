...
[MCP](https://modelcontextprotocol.io) server exposing the RemoteOkJobs SDK as
two agent tools — `remote-ok-jobs_list` and `remote-ok-jobs_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/remote-ok-jobs-mcp)
make build

# 2. Provide credentials via the environment
export REMOTE_OK_JOBS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user remote-ok-jobs \
  -- /absolute/path/to/remote-ok-jobs-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./remote-ok-jobs-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// remote-ok-jobs_list: first page of records
{ "entity": "get_all_job" }
{ "entity": "get_all_job", "query": { } }

// remote-ok-jobs_load: one record by id
{ "entity": "get_all_job", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/remote-ok-jobs-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user remote-ok-jobs \
     -- "$PWD"/dist/*/remote-ok-jobs-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `remote-ok-jobs_list` and `remote-ok-jobs_load` tools now appear
   in new sessions. Ask the agent to *"list get_all_job using remote-ok-jobs"*
   and it calls `remote-ok-jobs_list` with `{"entity":"get_all_job"}`.
...
```sh
export REMOTE_OK_JOBS_APIKEY=sk_live_xxx            # API key
export REMOTE_OK_JOBS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `remote-ok-jobs_list` tool
...
### Call the `remote-ok-jobs_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `remote-ok-jobs_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `remote-ok-jobs_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REMOTE_OK_JOBS_APIKEY` | API key sent with every request. |
| `REMOTE_OK_JOBS_BASE` | Optional override of the API base URL. |
...
```sh
./remote-ok-jobs-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"remote-ok-jobs_load","arguments":{"entity":"get_all_job","query":{"id":1}}}}'
```
