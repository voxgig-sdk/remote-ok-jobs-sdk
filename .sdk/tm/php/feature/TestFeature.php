...
// RemoteOkJobs SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RemoteOkJobsTestFeature extends RemoteOkJobsBaseFeature
{
    ...
    public function init(RemoteOkJobsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RemoteOkJobsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RemoteOkJobsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RemoteOkJobsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RemoteOkJobsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RemoteOkJobsParam::call($ctx, $k);
