...
// RemoteOkJobs SDK retry feature
...
class RemoteOkJobsRetryFeature extends RemoteOkJobsBaseFeature
{
    ...
    public function init(RemoteOkJobsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RemoteOkJobsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RemoteOkJobsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RemoteOkJobsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
