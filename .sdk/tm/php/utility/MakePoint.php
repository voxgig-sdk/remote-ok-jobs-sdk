...
// RemoteOkJobs SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class RemoteOkJobsMakePoint
{
    public static function call(RemoteOkJobsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof RemoteOkJobsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = RemoteOkJobsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = RemoteOkJobsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
