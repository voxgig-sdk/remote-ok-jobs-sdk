...
module RemoteOkJobsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RemoteOkJobsResponse.new({})
    ...
    if fetch_err
      ...
      response = RemoteOkJobsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RemoteOkJobsResponse.new(fetched)
    else
