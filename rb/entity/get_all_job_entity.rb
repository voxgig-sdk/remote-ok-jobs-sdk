...
class GetAllJobEntity
  ...
  def make
    opts = @_entopts.dup
    GetAllJobEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RemoteOkJobsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetAllJob, Hash] the current GetAllJob data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RemoteOkJobsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetAllJob fields)
  def match_get
  ...
  #
  # @param reqmatch [GetAllJobListMatch, Hash, nil] match filter (any subset of
  #   GetAllJob fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetAllJob>, Array] the matching GetAllJob items; raises RemoteOkJobsError on failure
