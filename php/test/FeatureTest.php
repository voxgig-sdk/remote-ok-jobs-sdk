...
class FtCtrl extends RemoteOkJobsControl
{
...
class FtClient
{
    ...
    private RemoteOkJobsUtility $utility;

    public function __construct(RemoteOkJobsUtility $utility, array $options)
    {
    ...
    public function get_utility(): RemoteOkJobsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RemoteOkJobsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new RemoteOkJobsUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (RemoteOkJobsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RemoteOkJobsConfig::make_feature($name);
    ...
    private static function buildUrl(RemoteOkJobsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RemoteOkJobsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RemoteOkJobsResponse($fetched);
        }
    ...
    private function fail(RemoteOkJobsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RemoteOkJobsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
