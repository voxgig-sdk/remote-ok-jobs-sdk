...
// RemoteOkJobs SDK GetAllJob entity
...
class GetAllJobEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetAllJobEntity($this->_client, $opts);
    }

    /**
     * @param GetAllJob|array $args GetAllJob data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RemoteOkJobsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetAllJob|array The current GetAllJob data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetAllJob fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RemoteOkJobsHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetAllJob fields).
     */
    ...
    /**
     * List GetAllJob items matching the given filter.
     *
     * @param GetAllJobListMatch|array|null $reqmatch Match filter (any subset
     *   of GetAllJob fields) as an assoc-array; GetAllJobListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetAllJob[]|array A list of GetAllJob items as assoc-arrays at
     *   the SDK boundary; throws RemoteOkJobsError on failure (item-5 convention).
     */
