...
local GetAllJobEntity = {}
GetAllJobEntity.__index = GetAllJobEntity


function GetAllJobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetAllJobEntity)
  self._name = "get_all_job"
  self._client = client
...
function GetAllJobEntity:get_name()
  return self._name
end


function GetAllJobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetAllJobEntity.new(self._client, opts)
end


function GetAllJobEntity:data_set(args)
  if args ~= nil then
...
function GetAllJobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetAllJobEntity:match_set(args)
  if args ~= nil then
...
function GetAllJobEntity:match_get()
...
function GetAllJobEntity:stream(action, args, callopts)
...
function GetAllJobEntity:_run_op(ctx, post_done)
  local utility = self._utility
