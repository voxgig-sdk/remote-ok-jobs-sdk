...
AQL-driven command-line client **and** interactive REPL for the RemoteOkJobs
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/remote-ok-jobs-cli)
make build

# 2. See usage (words, entities, env vars)
./remote-ok-jobs-cli --help
...
REMOTE_OK_JOBS_BASE=https://api.example.com ./remote-ok-jobs-cli list get_all_job

# 6. No arguments -> interactive REPL
./remote-ok-jobs-cli
remote-ok-jobs> list get_all_job
remote-ok-jobs> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/remote-ok-jobs-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/remote-ok-jobs-cli list get_all_job
   ```
...
Run with no arguments to open a REPL (prompt `remote-ok-jobs>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `get_all_job`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/remote-ok-jobs-cli`. |
...
(`./remote-ok-jobs-cli <expr>`) and interactively (the REPL), and expressions compose the
