// Command RemoteOkJobs-cli is a CLI + REPL for the RemoteOkJobs SDK,
...
const prompt = "remote-ok-jobs"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_all_job"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REMOTE_OK_JOBS_APIKEY (api key), REMOTE_OK_JOBS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REMOTE_OK_JOBS_APIKEY carries the API key and
	// REMOTE_OK_JOBS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REMOTE_OK_JOBS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REMOTE_OK_JOBS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRemoteOkJobsSDK(opts)
