...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.REMOTEOKJOBS_TEST_LIVE ||
    'TRUE' === process.env.REMOTEOKJOBS_TEST_OVERRIDE
  ) {
  ...
  m.REMOTEOKJOBS_TEST_EXPLAIN = process.env.REMOTEOKJOBS_TEST_EXPLAIN || m.REMOTEOKJOBS_TEST_EXPLAIN
