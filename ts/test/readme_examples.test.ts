...
//     (\`new RemoteOkJobsSDK(...)\` / \`RemoteOkJobsSDK.test(...)\` become
//     \`RemoteOkJobsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RemoteOkJobsSDK'
...
const TEST_SEED = {"entity":{"get_all_job":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RemoteOkJobsSDK(...)\` and any
// \`RemoteOkJobsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RemoteOkJobsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RemoteOkJobsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
